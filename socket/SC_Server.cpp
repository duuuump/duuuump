...
BOOL ServerSC::Close(SOCKET &socket)
{
	return CloseSocket(&socket, false);
}

void ServerSC::Replay(string msg)
{

	set<SOCKET>::iterator iter = clients.begin();
	while (iter != clients.end())
	{
		int len;
		DataFrame* frame = new DataFrame(msg.c_str(), msg.size());
		byte* res = frame->GetBytes(len);
		this->Send(*iter, res, len);
		delete frame;
		frame = NULL;
		iter++;
	}
